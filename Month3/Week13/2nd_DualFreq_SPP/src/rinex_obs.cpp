...
vector<ObsEpoch> parseRinexAllEpochs(const std::string& filename)
{
	...
	// NEW: resolve dynamic column indices
	...
	cout << "Resolved observable indices: C1W=" << idx_C1W
	     << "  C2W=" << idx_C2W << endl;

	int col_C1 = (idx_C1W >= 0) ? (3 + 16 * idx_C1W) : 3;   // fallback to old hard-coded 3
	...
	while (getline(file, line))
	{
		...
		for (int i = 0; i < n_satss; i++)
		{
			...
			// NEW: C2W pseudorange (dynamic column, optional)
